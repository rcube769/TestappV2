...
import { Trash2, Star, Candy, Sparkles, Shield } from 'lucide-react'
...
export default function AdminPanel({ ratings, onDelete, isDeleting }: AdminPanelProps) {
  return (
    <Card className="border-red-200 shadow-lg">
      <CardHeader className="bg-gradient-to-r from-red-100 to-pink-100 border-b border-red-200">
        <CardTitle className="flex items-center gap-2 text-lg">
          <Shield className="w-5 h-5 text-red-600" />
          Admin Panel - All Ratings ({ratings.length})
        </CardTitle>
      </CardHeader>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 space-y-2">
                      ...
                      <p className="text-xs text-gray-400">
                        {rating.address && <span className="block">{rating.address}</span>}
                        {new Date(rating.created_date).toLocaleDateString()} • User: {rating.userFingerprint.slice(0, 8)}
                      </p>
                    </div>
                    <Button
                      variant="ghost"
